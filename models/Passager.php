<?php
    class Passager { 
        ...
        static public function update($data3){
                ...
                // $cin_passager = $_SESSION['cin'];
                // $query3 = DB::connect()->prepare("SELECT * FROM `passager` WHERE `id_user_created` =".$id_user_created."  AND cin_passager = ".$cin_passager."");
                // $query3 ->execute();
                // if($query3->rowCount() > 0){
                //     while($row = $query3->fetch(PDO::FETCH_ASSOC)){
                //         $idpassager = $row["id_passager"];
                //         $_SESSION['idpassager'] = $idpassager;
                //     }
                // }

                // $stmt = DB::connect()->prepare('UPDATE passager SET nom_passager = :nom,prenom_passager = :prenom,email_passager = :email,cin_passager = :cin WHERE id_user_created = :id AND id_passager = :id_passager');
                // // bach kanrbet mabin les parametre el les donne dyalhom
                // $stmt->bindParam(':id',$_SESSION['idUser']);
                // $stmt->bindParam(':nom',$data3['nom']);
                // $stmt->bindParam(':prenom',$data3['prenom']);
                // $stmt->bindParam(':email',$data3['email']);
                // $stmt->bindParam(':cin',$data3['cin']);
                // $stmt->bindParam(':id_passager',$_SESSION['idpassager']);
                // // die(print_r($data));
                // if($stmt->execute()) {
                //     // hena khedmet lcontroler
                //     return 'ok';
        ...
        // SELECT p.* FROM passager p,user u WHERE p.cin_passager = u.cin And p.id_user_created = u.id AND u.id =9

        // static public function getPasager($data){
		// 	$id = $data['id'];
		// 	try{
        ...
        function passager_show_id($id) {
            ...
            // $stmt = DB::connect()->prepare("SELECT * from passager where id_passager='$id'");
			// $stmt ->execute();
